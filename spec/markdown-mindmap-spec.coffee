...
describe "Markdown Mindmap package", ->
  ...
  expectPreviewInSplitPane = ->
    ...
    waitsFor "Markdown Mindmap to be created", ->
  ...
  describe "when a preview has not been created for the file", ->
    it "displays a Markdown Mindmap in a split pane", ->
      waitsForPromise -> atom.workspace.open("subdir/file.markdown")
      runs -> atom.commands.dispatch workspaceElement, 'markdown-mindmap:toggle'
      expectPreviewInSplitPane()
    ...
    describe "when the editor's path does not exist", ->
      it "splits the current pane to the right with a Markdown Mindmap for the file", ->
  ...
  describe "when the Markdown Mindmap view is requested by file URI", ->
    it "opens a preview editor and watches the file for changes", ->
      waitsForPromise "atom.workspace.open promise to be resolved", ->
        atom.workspace.open("markdown-mindmap://#{atom.project.getDirectories()[0].resolve('subdir/file.markdown')}")
  ...
  describe "when the editor's grammar it not enabled for preview", ->
    it "does not open the Markdown Mindmap", ->
  ...
  describe "when the editor's path changes on #win32 and #darwin", ->
    it "updates the preview's title", ->
      ...
      waitsFor ->
        preview.getTitle() is "file2.md Mindmap"

      runs ->
        expect(titleChangedCallback).toHaveBeenCalled()

  describe "when the URI opened does not have a markdown-mindmap protocol", ->
  ...
  describe "when markdown-mindmap:copy-html is triggered", ->
  ...
  describe "GitHub style Markdown Mindmap", ->
    beforeEach ->
